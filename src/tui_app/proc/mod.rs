...
impl Proc {
    pub fn new(cfg: &ProcConfig, tx: UnboundedSender<(usize, ProcEvent)>, size: Rect) -> Self {
    ...
    fn spawn_new_inst(&mut self) {
        ...
        let spawned = Inst::spawn(self.id, self.cmd.clone(), self.tx.clone(), &self.size);
    ...
    pub fn lock_vt(&self) -> Option<std::sync::RwLockReadGuard<'_, vt100::Parser>> {
        match &self.inst {
            ProcState::None => None,
    ...
    pub fn lock_vt_mut(&mut self) -> Option<std::sync::RwLockWriteGuard<'_, vt100::Parser>> {
        match &self.inst {
            ProcState::None => None,
                ...
                MouseEventKind::Down(btn) => match btn {
                    ...
                    MouseButton::Right => {
                        self.copy_mode = match std::mem::take(&mut self.copy_mode) {
                            CopyMode::None(_) => unreachable!(),
                            CopyMode::Start(screen, start) | CopyMode::Range(screen, start, _) => {
...
impl Proc {
    pub fn handle_cmd(&mut self, cmd: ProcCmd) {
        match cmd {
            ...
            ProcCmd::CopyModeEnd => {
                self.copy_mode = match std::mem::take(&mut self.copy_mode) {
                    CopyMode::Start(screen, start) => CopyMode::Range(screen, start.clone(), start),
                    other => other,
                };
            }

...
fn get_config_dir() -> Option<PathBuf> {
    if let Some(portable_dir) = get_portable_config_dir() {
        ...
        Some(portable_dir)
    } else if let Some(config_dir) = get_home_cocmd_dir() {
        // $HOME/.cocmd
        debug!("detected config directory in $HOME/.cocmd");
        Some(config_dir)
    } else if let Some(config_dir) = get_home_config_cocmd_dir() {
        // $HOME/.config/cocmd
        debug!("detected config directory in $HOME/.config/cocmd");
        Some(config_dir)
    } else if let Some(legacy_mac_dir) = get_legacy_mac_dir() {
        // Legacy macOS location in ~/Library/Preferences/cocmd
        debug!(
...
fn get_portable_config_dir() -> Option<PathBuf> {
    let cocmd_exe_path = std::env::current_exe().expect("unable to obtain executable path");
    let exe_dir = cocmd_exe_path.parent();
    if let Some(parent) = exe_dir {
        let config_dir = parent.join(".cocmd");
...
fn get_home_cocmd_dir() -> Option<PathBuf> {
    if let Some(home_dir) = dirs::home_dir() {
        let config_cocmd_dir = home_dir.join(".cocmd");
        if config_cocmd_dir.is_dir() {
            return Some(config_cocmd_dir);
        }
    }
    None
}

fn get_home_config_cocmd_dir() -> Option<PathBuf> {
    if let Some(home_dir) = dirs::home_dir() {
        let home_cocmd_dir = home_dir.join(".config").join("cocmd");
        if home_cocmd_dir.is_dir() {
            return Some(home_cocmd_dir);
        }
    }
    None
}
...
fn get_default_config_path() -> PathBuf {
    ...
    config_dir.join("cocmd")
}

// Due to the original behavior of the dirs crate, cocmd placed the config
// directory in the Preferences folder on macOS, but this is not an optimal
// approach.
// For more context, see: https://github.com/Moshe-Roth/cocmd/issues/611
fn get_legacy_mac_dir() -> Option<PathBuf> {
    if cfg!(target_os = "macos") {
        if let Some(preferences_dir) = dirs::preference_dir() {
            let cocmd_dir = preferences_dir.join("cocmd");
            if cocmd_dir.is_dir() {
                return Some(cocmd_dir);
            }
        }
    }
    None
...
fn get_portable_runtime_path() -> Option<PathBuf> {
    ...
    let exe_dir = cocmd_exe_path.parent();
    if let Some(parent) = exe_dir {
        let config_dir = parent.join(".cocmd-runtime");
        return Some(config_dir);
    }
    None
}
...
fn get_legacy_runtime_dir() -> Option<PathBuf> {
    ...
    let cocmd_dir = data_dir.join("cocmd");
    if is_legacy_runtime_dir(&cocmd_dir) {
        Some(cocmd_dir)
    } else {
        None
    }

...
const INSTALLER_NAME: &str = "Cocmd-Win-Installer";
...
fn main() {
    ...
    let script_repackages_path = project_path
        .join("scripts")
        .join("repackages")
        .join("windows");
    let template_path = script_repackages_path.join("setupscript.iss");
    if !template_path.is_file() {
        panic!("InnoSetup template not found");
    }

    let template =
        std::fs::read_to_string(template_path).expect("unable to read InnoSetup template");

    let cocmd_toml_path = project_path.join("cocmd").join("Cargo.toml");
    if !cocmd_toml_path.is_file() {
        panic!("could not find cocmd Cargo.toml file");
    }

    let cocmd_toml_str =
        std::fs::read_to_string(cocmd_toml_path).expect("unable to read Cargo.toml file");
    let cocmd_toml = cocmd_toml_str
        .parse::<Value>()
    ...
    iss_setup = iss_setup.replace(
        "{{{app_version}}}",
        cocmd_toml["package"].as_table().unwrap()["version"]
            .as_str()
            .unwrap(),
    );
    iss_setup = iss_setup.replace(
        "{{{app_url}}}",
        cocmd_toml["package"].as_table().unwrap()["homepage"]
            .as_str()
            .unwrap(),
    );
    iss_setup = iss_setup.replace(
    ...
    iss_setup = iss_setup.replace(
        "{{{app_icon}}}",
        &dunce::canonicalize(script_repackages_path.join("icon.ico"))
            .unwrap()
            .to_string_lossy()
            .to_string(),
    );
    iss_setup = iss_setup.replace(
        "{{{cli_helper}}}",
        &dunce::canonicalize(script_repackages_path.join("cocmd.cmd"))
            .unwrap()
            .to_string_lossy()
            .to_string(),
    );
    ...
    iss_setup = iss_setup.replace(
        "{{{executable_path}}}",
        &dunce::canonicalize(&repackages_dir.join("cocmdd.exe"))
            .unwrap()
            .to_string_lossy()
            .to_string(),
    );

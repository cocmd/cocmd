...
const TARGET_DIR: &str = "target/windows/repackages";
...
fn main() {
    ...
    if !exec_file.is_file() {
        panic!("expected cocmd binary, found none in {:?}", exec_file);
    }
    std::fs::copy(exec_file, target_dir.join("cocmdd.exe")).unwrap();

    // Create the CLI wrapper
    std::fs::write(target_dir.join("cocmd.cmd"), r#"@"%~dp0cocmdd.exe" %*"#).unwrap();
}

...
fn main() {
  ...
  // Create the launcher
  std::fs::write(target_dir.join("START_ESPANSO.bat"), r#"start cocmdd.exe launcher"#).unwrap();

  // Create the necessary folders
  std::fs::create_dir_all(target_dir.join(".cocmd")).expect("unable to create data directory");
  std::fs::create_dir_all(target_dir.join(".cocmd-runtime")).expect("unable to create runtime directory");

  std::fs::write(target_dir.join("README.txt"), r##"Welcome to Cocmd (Portable edition)!

To start cocmd, you can double click on "START_ESPANSO.bat"  

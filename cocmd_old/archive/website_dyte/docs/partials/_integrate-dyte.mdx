import ThemedImage from '@theme/ThemedImage';
import useBaseUrl from '@docusaurus/useBaseUrl';

<span>
  This topic provides a simple guide to integrating cocmd's {props.product} into
  your app, enabling you to add high-quality real-time communication features.
</span>

The following diagram illustrates the steps involved and demonstrates the interaction between cocmd's backend and frontend during the integration process.

![Diagram explaining how cocmd works](/static/home/how-dyte-works-v2.png)

## Step 1: Register in the Developer Portal

To get started with cocmd, you must first register in the [Developer portal](https://dev.cocmd.ai).

## Step 2: Get your API Keys

Copy the [API Keys](https://dev.cocmd.ai/apikeys) for your organization from
the Developer portal. The API Key is unique to your organization and is
required to use cocmd REST APIs such as when instantiating a meeting.

<ThemedImage
  alt="API key"
  width="600"
  height="300"
  sources={{
    light: useBaseUrl('/static/dev-portal/api-key-light.png'),
    dark: useBaseUrl('/static/dev-portal/api-key-dark.png'),
  }}
/>

## Step 3: Create Presets

A preset is a set of permissions and UI configurations that would be
applied to a participant. For more information, see [Add Presets](/guides/capabilities/presets).

1. In the [Developer Portal](https://dev.cocmd.ai/), click **Presets**, and click **Create New**.
   <ThemedImage
     alt="Create preset"
     width="600"
     height="300"
     sources={{
       light: useBaseUrl('/static/dev-portal/create-preset-light.png'),
       dark: useBaseUrl('/static/dev-portal/create-preset-dark.png'),
     }}
   />
2. <span>
     In the <b>Select Meeting Type</b>, select <b>{props.preset}</b>.
   </span>
   <ThemedImage
     alt="Create preset"
     width="600"
     height="300"
     sources={{
       light: useBaseUrl('/static/preset/' + props.presetType + '.png'),
       dark: useBaseUrl('/static/preset/' + props.presetType + '.png'),
     }}
   />

3. Set permissions and UI configurations.

4. Once you’ve made all the changes that you need to add to your preset, click **Save** at the right-corner.
   <img
     src="/static/preset/savepreset.png"
     width="600"
     height="300"
     alt="preset-save"
   />
5. Enter the name for your preset and click **Save**.

<span>{props.savePresetInfo}</span>

## Step 4: Create a Meeting

Create a cocmd meeting using the [Create Meeting API](/api#/operations/create_meeting). This API returns a unique identifier for your meeting.

## Step 5: Add Participants

After you've created the meeting, add participants to the meeting using the [Add Participant API](/api#/operations/add_participant).

The preset created in the previous step must be passed in the body of the Add Participant API request. Specify the name of the preset in the `preset_name` parameter.

This API returns the authentication `token` that is required to initialize your frontend SDKs.

cocmd meetings do not have a date or time associated with them, so you can create
them in advance or just when users need to join the meeting. The idea is that
your backend controls when to distribute authentication `token` and thus you can share the
tokens with the users at the scheduled time.

## Next Step

<Card
  title="Initizlize Your Frontend"
  to={'/guides/' + props.productSlug + '/client-setup'}
/>

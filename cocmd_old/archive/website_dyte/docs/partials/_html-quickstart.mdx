...
<span>
  This quickstart shows how to use cocmd's {props.product} to your web
...
### Call the `cocmdClient.init()` method from the `web-core` package and pass the `authToken`:

This will establish the connection with the cocmd server. You should keep the returned `meeting` object.

...
| isMe                 | boolean                                | Checks if the participant is the current participant, in which case it can be treated as [`cocmdSelfParticipant`](./self-participant) |
...
| metadata.view_type   | Enum ([cocmdViewTypes](#dyteviewtypes)) | The type of meeting view that the participant is in                                                                                  |
...
`cocmdViewTypes` is an enum that enumerates the different kind of meeting views,
that are available to a participant in cocmd meeting.

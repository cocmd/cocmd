...
cocmd uses VP9 codec by default, but switches to H264/VP8 depending on
participants' video codec support. VP9 is a superior codec but not all browsers
support it. If you set `autoTune` in `meetingConfig` to `false`, cocmd will use
...
to `cocmdMeeting`. `onError` expects a function which has a single param of type
...
type `cocmdPlugin`, whose prototype is defined below in the reference section and
...
meeting controls. This is possible with cocmd, with the minor caveat that you
...
`cocmdParticipant`, whose prototype is defined below in the reference section and
can be directly used as a type if you prefer using TypeScript bindings. There
will be exactly one element of type `cocmdSelfParticipant`, which refers to the
current participant.

## Controlling individual participants and actions

cocmd client SDK provide methods and properties to help you control participant
...
`Array[cocmdParticipant|cocmdSelfParticipant]` where all meeting participants are
[`cocmdParticipant`](./reference/participant) and the current participant is
[`cocmdSelfParticipant`](./reference/self-participant).
...
Get unique ID for every participant. This ID is generated by cocmd when a
participant joins the meeting. This ID is used for referring to a participant
within cocmd, such as for
...
that participant as an instance of type `cocmdSelfParticipant`, which is a
subclass of type `cocmdParticipant`. This unlocks a few additional actions that
...
more about these on the [`cocmdErrors`](./reference/dyte-errors) page in the
reference section.
...
```tsx
import { cocmdMeeting, cocmdErrors } from "@dyte/client";
...
<cocmdMeeting
    ...
    onError={(error) => {
        if (error = cocmdErrors.CLIENTID_INVALID) {

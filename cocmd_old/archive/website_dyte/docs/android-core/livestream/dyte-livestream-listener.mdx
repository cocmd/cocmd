...
You can listen to livestream events by attaching a listener by calling `addLivestreamEventsListener` on `dyteMobileClient` object where `dyteMobileClient` is an instance of `cocmdMobileClient()`.

```kotlin
val dyteLiveStreamEventsListener = object : cocmdLiveStreamEventsListener {
  ...
  override fun onLiveStreamStateUpdate(data: cocmdLivestreamData) {

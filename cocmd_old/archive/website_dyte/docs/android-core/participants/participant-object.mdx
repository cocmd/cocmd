...
```kotlin
dyteParticipant.addParticipantUpdateListener(object : cocmdParticipantUpdateListener {
      override fun onAudioUpdate(participant: cocmdMeetingParticipant, isEnabled: Boolean) {
        super.onAudioUpdate(participant, isEnabled)
        // on audio update
      }

      override fun onVideoUpdate(participant: cocmdMeetingParticipant, isEnabled: Boolean) {
        super.onVideoUpdate(participant, isEnabled)
        // on video update
      }

      override fun onPinned(participant: cocmdMeetingParticipant) {
        super.onPinned(participant)
        // when participant is pinned
      }

      override fun onUnpinned(participant: cocmdMeetingParticipant) {
        super.onUnpinned(participant)
        // when participant is un-pinned
      }

      override fun onScreenShareStarted(participant: cocmdMeetingParticipant) {
        super.onScreenShareStarted(participant)
        // when participant start to screenshare
      }

      override fun onScreenShareEnded(participant: cocmdMeetingParticipant) {

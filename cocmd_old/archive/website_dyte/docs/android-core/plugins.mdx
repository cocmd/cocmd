...
to get webview from cocmdPlugin one needs to first acivate a plugin. To do that
...
callback `cocmdPluginEventsListener`. You can subscribe to this events by calling
...
```kotlin
    meeting.addMeetingRoomEventsListener(object :
      ...
      override fun onPluginActivated(plugin: cocmdPlugin) {
      super.onPluginActivated(plugin)
      // your code to handle plugin activation
    }

    override fun onPluginDeactivated(plugin: cocmdPlugin) {

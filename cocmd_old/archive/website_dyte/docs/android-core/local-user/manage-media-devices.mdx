...
val audioDevices: List<cocmdAudioDevice> = meeting.self.getAudioDevices();

// Get all video devices
val videoDevices: List<cocmdVideoDevice> = meeting.self.getVideoDevices();
```

## Set audio device

To set a device as an active device, you can call `setAudioDevice` method. This
takes a `cocmdAudioDevice` object.
...
This takes a `cocmdVideoDevice` object.

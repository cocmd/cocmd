...
```kotlin {3,4}
val meetingInfo = cocmdMeetingInfo(
...
After joining the room, if the audio and video tracks were disabled during the `cocmdMobileClient` initialization, you can enable them easily by using the `enableAudio()` and `enableVideo()` functions:
...
```kotlin
// switch camera
meeting.localUser.setVideoDevice(videoDevice: cocmdVideoDevice)
```

...
function Meeting() {
  ...
  function ParticipantTile({ participant, meeting }) {
    return (
      ...
      >
        <cocmdNameTag participant={participant} meeting={meeting}>
          <cocmdAudioVisualizer participant={participant} />
        </cocmdNameTag>
        <cocmdAvatar size="sm" participant={participant} />
      </cocmdParticipantTile>
    );
  }
...
cocmd has ready to use grid components, that you can use out of the box
...
- Parent Grid component - `cocmdGrid`
- Child Grid components each having a different layour - `cocmdSimpleGrid`,
  `cocmdMixedGrid`, `cocmdSpotlightGrid`

#### Parent Grid Component

The `cocmdGrid` component is a monolith component which handles all grid related
...
function Example() {
  ...
  return (
    <div style={{ width: '100%', height: '400px' }}>
      <cocmdGrid meeting={meeting} aspectRatio="16:9" gap="8" size="sm" />
    </div>
  );
}
```
...
function Example() {
  ...
  return (
    <div>
      ...
      >
        <cocmdButton onClick={add}>Add</cocmdButton>
        <cocmdButton variant="danger" onClick={remove}>
          Remove
        </cocmdButton>
      </div>
      <div style={{ width: '100%', height: '400px' }}>

...
cocmd's React UI Kit is a prebuilt design library of UI components that makes it
...
- Uses a layered design structure, allowing you to start with one prebuilt
  component and slowly add further layers as the needs evolve. For example, you can create a full fledged meeting experience using just one UI component, cocmdMeeting. The [cocmdMeeting component](/react-ui-kit#components-of-dytemeeting) contains all the necessary features and subcomponents to create a default meeting UI.
...
cocmd's UI Kit offers a couple of ways to get started with your live video and
voice calling applications.

- **Use prebuilt components**: You can use prebuilt `cocmdMeeting` component to
...
- **Build your own UI**: cocmd also offers the flexibility to build your own UI
...
## Components of `cocmdMeeting`

The cocmdMeeting component can be broken down into 3 major subcomponents:
...
The grid displays the video feeds of meeting participants. The grid component in cocmd's UI Kit adjusts the layout of video feeds based on the meeting's participant count, ensuring a smooth meeting experience. It provides a variety of layouts and variants.
...
- cocmd UI Kit is built using
  ...
  - **Encapsulation**: Web components utilize [Shadow DOM](https://developer.mozilla.org/en-US/docs/Web/Web_Components/Using_shadow_DOM), which provides DOM and style encapsulation, isolating the styling of each component. This ensures that the CSS on your page does not interfere with cocmd's component styling and vice versa.
...
- cocmd's React UI Kit also provides multiple
  ...
  experience when integrating cocmd's UI Kit in your React project. You can

...
1. Set up `cocmdProvider`. You need it to import the `cocmdProvider` from the
   `dytesdk/react-web-core`. cocmdProvider basically is a hook wrapper on
   dytesdk/web-core. This provides a meeting object to child components.
2. Initialize the cocmd client. Use the `usecocmdClient()` hook and `initMeeting`
...
[cocmdMeeting](/react-ui-kit/components/dyte-meeting).

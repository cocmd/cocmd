---
sidebar_position: 4
description: Details about the Design System in cocmd's UI Kit.
---
...
cocmd's UI Kit uses Atomic design as a principle to bring logic and structure to
individual screens. Read this
[blog post](https://cocmd.ai/blog/new-dyte-video-sdk-design-system/) to get a
...
Let's look at how you can customize your UI with cocmd's UI Kit.

cocmd provides the `providecocmdDesignSystem()` utility to programmatically
...
function Example() {
  ...
  useEffect(() => {
    // providecocmdDesignSystem(node, designTokens)
    providecocmdDesignSystem(meetingEl.current, {
      googleFont: 'Lobster',
  ...
  return (
    <div style={{ height: '400px' }}>
      <cocmdMeeting meeting={meeting} ref={meetingEl} mode="fill" />
    </div>
  );
}
```
...
You can edit this value in two ways with the providecocmdDesignSystem utility.
...
This is because the `providecocmdDesignSystem()` utility sets the color you pass

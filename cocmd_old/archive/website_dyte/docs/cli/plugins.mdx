...
You can publish plugins for your organization on cocmd. We manage only the
client-side code for plugins, the backend for a plugin (if any) **will not** be
hosted by cocmd. Therefore, server-side templating is not supported in plugins.

Every functional cocmd plugin requires a configuration file, which is named
...
5. Run `dyte plugins publish -l` or `dyte plugins publish --latest` to upload
   and serve your plugin on cocmd.

## Usage

### Creating a plugin

You can use the following subcommand to create a new cocmd plugin. This sends an
...
The `publish` subcommand handles publishing a plugin on cocmd. You must run this
...
show up on the cocmd plugins panel. The `pluginId` is required.
...
`version list` can be used to list all versions of your cocmd plugin.
...
`version delete` can be used to delete a particular version of your cocmd plugin.

...
The cocmdPlugin class is the root class of Plugin SDK. It is the main entry point
of the SDK. It is the only class that you need to instantiate in order to use
the SDK.

A plugin object can be created using `cocmdPlugin.init()` method. The `init()`
...
| pluginId  | string   | ID generated using cocmd CLI.                         | auto-filled by client SDK. | false        |
| ready     | boolean  | `true` when the plugin initialization has completed. | `true`                     | false        |

```jsx
import cocmdPlugin from "@dytesdk/plugin-sdk";
...
const plugin = cocmdPlugin.init();

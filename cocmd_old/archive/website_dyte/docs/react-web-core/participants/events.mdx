...
```ts
const viewMode = usecocmdSelector((meeting) => meeting.participants.viewMode);
```

### Page change

```ts
const pageChanged = usecocmdSelector(
...
```ts
const activeSpeaker = usecocmdSelector(
...
```ts
const pinnedParticipants = usecocmdSelector(
...
```ts
const videoEnabled = usecocmdSelector((m) =>
...
```ts
const audioEnabled = usecocmdSelector((m) =>
...
```ts
const screenShareEnabled = usecocmdSelector((m) =>
...
```ts
const screensharingParticipant = usecocmdSelector((m) =>

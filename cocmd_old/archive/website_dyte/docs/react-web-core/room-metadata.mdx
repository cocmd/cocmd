...
```ts
const meetingconnected = usecocmdSelector(
...
const meetingTitle = usecocmdSelector((meeting) => meeting.meta.meetingTitle);
useEffect(() => {
  console.log('I got meetingTitle as: ', meetingTitle);
}, [meetingTitle]);

const meetingStartedTimestamp = usecocmdSelector(

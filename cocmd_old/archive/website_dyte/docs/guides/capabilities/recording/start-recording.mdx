...
This topic explains how to use cocmd to implement composite recording.
...
There are two ways to start a recording a cocmd meeting:
...
cocmd stores recordings for a period of 7 days, after which they will expire and no longer be accessible. It is important to either download a copy of your recording or [set up storage](https://docs.cocmd.ai/guides/recording/custom-cloud-storage#set-storage-configuration) before the link expires.
...
`storage-config' using the [Developer Portal](https://dev.cocmd.ai/).
...
will be started as soon as a meeting starts on cocmd, i.e, when the first

...
When you join a cocmd meeting, the meeting layout is automatically designed to optimize your experience. This includes focusing on shared content and highlighting active speakers, while participants are shown in small thumbnail views. When you start recording the meeting, it is recorded with the same layout using the default UI kit component called [cocmdGrid](/react-ui-kit/components/dyte-grid).

If you wish to have a customized layout for your recording application, cocmd's custom recording SDKs provide the flexibility to tailor the appearance of your recordings according to your preferences. You can choose from options like:
...
Let’s dive in to understand the steps involved in creating a custom appearance for your cocmd recording app.

The custom recording SDKs are used on top of the [UI kit](/react-ui-kit) or [Vanilla JS](/ui-kit). The `cocmdRecording` class is used for managing the recording functionality within the SDK.

## Constructor

`constructor(options)`

Creates an instance of the `cocmdRecording` class.
...
```js
init(client: cocmdClient)
```

Initiates the SDK by providing a `cocmdClient` object. Call this after creating the meeting object and before calling `meeting.joinRoom()`.
...
Perform the following steps to create the recording app for your cocmd meetings.
...
### Step 2: Import the `cocmdRecording` object

```js
import cocmdRecording from '@dytesdk/recording-sdk';
```

### Step 3: Create the `cocmdRecording` object

```js
const recordingSdk = new cocmdRecording(options);
```
...
Once you've created the app, deploy it using a platform like [Vercel](https://vercel.com/docs/concepts/get-started). Make sure to note the URL where you have deployed the app, as you will have to enter this URL in cocmd's recording API.

...
cocmd uses webhooks to notify your application when an event happens. Webhooks
...
## How does cocmd use Webhooks?

A webhook enables cocmd to push real-time updates to your applications. When the
event occurs, cocmd makes an HTTP request (usually a POST) to the URL you
...
systems. To learn more, see cocmd webhook events overview.
...
| `dyte-signature`  | cocmd verifies the webhook events it sends to your endpoints by including a signature in the dyte-signature header of each event. This allows you to confirm that the event came from cocmd and not a malicious server impersonating cocmd. See [Verify Webhooks Using cocmd's Signature](/guides/capabilities/webhooks/signatures#verify-using-dytes-signature). |

...
3. Handle requests from cocmd by parsing each event object and returning 2xx
   response status codes.
4. Deploy your webhook endpoint so it’s a publicly accessible HTTPS URL.
5. Register your publicly accessible HTTPS URL using the cocmd
   [developer portal](https://dev.cocmd.ai) or Webhook APIs.
...
### Step 3: Handle requests from cocmd

Your endpoint must be configured to receive events for the type of event
notifications you want to receive. cocmd sends events to your webhook endpoint as
...
our [dev portal](https://dev.cocmd.ai) or by making an API request. For example,
to receive all events, you can make a request like:

```bash
curl --location --request POST 'https://api.cocmd.ai/v2/webhooks' \
...
cocmd webhooks have built-in retry methods for `3xx`, `4xx` or `5xx` response
status codes. If cocmd doesn't receive a `2xx` response status code for an event within 3 seconds,
we mark the event as failed and retry up to 5 times.

### Step 4: Secure your webhooks (recommended)

Use [webhook signatures](/guides/capabilities/webhooks/signatures) to verify that cocmd
generated a webhook request and that it didn't come from a malicious server
pretending to be cocmd.
...
[cocmd developer portal](https://dev.cocmd.ai) or

...
cocmd's UI kit provides a convenient solution for customizing the UI of your cocmd meetings including control bar. The control bar is an essential component of any meeting platform, allowing users to access various meeting controls such as audio and video settings, screen sharing, recording, and more. Using cocmd's UI kit you can customize the control bar, for example, add a hand-raise button, rename a button, and so on.

This guide will take you through the process of adding a hand-raise button to the control bar of your meeting.

## Prerequisites

- Review [Components of cocmdMeeting](/react-ui-kit#components-of-dytemeeting)
- Review the [Get Started with cocmd Prebuilt Components](/react-ui-kit/quickstart#step-2-get-started-with-dyte-prebuilt-components) and [Example](/react-ui-kit/quickstart#example-using-prebuilt-dytemeeting-component) sections in the React UI kit quickstart
- Integrate [cocmdParticipantsAudio](/react-ui-kit/basics/integrate#dyteparticipantsaudio)
- Integrate [cocmdNotifications](/react-ui-kit/basics/integrate#dytenotifications)
...
import { usecocmdClient, cocmdProvider } from '@dytesdk/react-web-core';
import {
...
if (!client) return (<div>Loading cocmd Meeting...</div>);

return (
  <div className="dyte-meeting">
  <cocmdProvider value={client}>
      <cocmdParticipantsAudio meeting={client} />
      <cocmdNotifications
        meeting={client}
        config={{
          config: {
      ...
      />
      <cocmdHeader meeting={client} />
      <div className="grid-container">
        <cocmdGrid meeting={client} style={{ height: '100%' }} />
      </div>
      <div class="controlbar">
        <cocmdMicToggle meeting={client} />
        <cocmdCameraToggle meeting={client} />
        <cocmdSettingsToggle meeting={client} />
      </div>
  </cocmdProvider>
  </div>
);
...
Once you've created your cocmd meeting, import the `cocmdControlbarButton` and create a state to manage the UI.

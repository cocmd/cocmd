...
4. Initialize the cocmd client using the `cocmdClient.init({ authToken })`. It returns the meeting object.
...
In order to replace cocmd's default icon set with your own, you can pass the link to your icon set in the `dyte-meeting` component like so.

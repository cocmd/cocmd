...
# cocmd REST API Quickstart

cocmd’s API follows the
...
This topic describes how to get started with cocmd v2 REST APIs.

Before getting started with this quickstart guide, we recommend that you read
[Getting Started with cocmd](/getting-started) to familiarize yourself with cocmd.

## Objectives

In this quickstart you’ll learn:

- How to access cocmd APIs
- How to authenticate the requests
- How to create presets
- How to add voice and video calls to your applications using cocmd APIs

## Initial Setup

Perform the following steps to get started with cocmd APIs:

### Step 1: Access APIs

You can access all of our v2 API endpoints at `https://api.cocmd.ai/v2/`.
For the complete list of routes, see [API documentation](/api/?v=v2).

### Step 2: Authenticate using your API Keys

cocmd's APIs use API Keys to authenticate requests via
...
1.  First, register on our [Developer Portal](https://dev.cocmd.ai).
...
[Create presets](https://dev.cocmd.ai/roles-presets) using the Developer portal.
A preset is a set of permissions and UI configurations that is applied to a
participant.

## Add Video Calls to your Application using cocmd APIs

This section focuses on how to add voice and video calls to your applications
using cocmd APIs. A typical workflow includes the following steps:
...
You must first create a meeting in order to begin using cocmd for voice and video
calls.
...
```bash
curl --request POST \
  --url https://api.cocmd.ai/v2/meetings \
...
```bash
curl --request POST \
  --url https://api.cocmd.ai/v2/meetings/<meetingId>/participants \
  --header 'Authorization: Basic ZGV2QGR5dGUuaW46MTIzNDU2Nzg5YXNkaGZnaGo=' \
  --header 'Content-Type: application/json' \
  --data '{
  "name": "Mary Sue",
...
You can manage your recordings in cocmd using `Basic` or `Bearer` authentication
...
```bash
curl --request POST \
  --url https://api.cocmd.ai/v2/recordings \
...
cocmd uses webhooks to notify your application when an event occurs. Webhooks are
especially useful for asynchronous events like when a recording is uploaded or a
new participant joins the meeting.

cocmd can push real-time updates to your application via a webhook. Events are
...
```bash
curl --request POST \
  --url https://api.cocmd.ai/v2/webhooks \
  --header 'Authorization: Basic NzBjNWQzOTEtNGJhYy00Y2YzLTk5MDctYmVjMjA1Nzk4YWRiOmQyNzBmYjJmOGNiNGUzZWY1MGI1' \
  --header 'Content-Type: application/json' \
  --data '{
  "name": "Attendance",

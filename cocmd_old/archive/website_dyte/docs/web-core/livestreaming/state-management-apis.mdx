...
This topic talks about how you can use livestreaming stage management APIs. Using cocmd's live-streaming stage management APIs you can easily manage stage requests, such as leave and join stage, manage permissions, kick participants and so on.
...
cocmd's livestreaming stage management APIs provides useful properties to help you manage your stage effectively. One of these properties is the `status` attribute, which can take on various values based on the user's activity. The status can assume the following values:

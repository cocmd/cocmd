...
- recordingState: `cocmdRecordingState`, Indicates the current recording
  state of the meeting.

To listen recording status updates, you need to listen for
`onMeetingRecordingStateUpdated()` event. This returns `cocmdRecordingState`
object with it.
...
`onMeetingRecordingStateUpdated` from `cocmdMeetingRoomEventsListener`.

```dart

class RecordingListener with cocmdRecordingEventsListener {
  ...
  @override
  void onMeetingRecordingStateUpdated(cocmdRecordingState recordingState) {

...
This quickstart shows how to use cocmd's Flutter Core SDK to add live video and
...
1. To install the SDK, add the cocmd Core Flutter SDK dependency into the
   `pubspec.yaml` file.
...
```dart
final dyteClient = cocmdMobileClient();
```

## Step 3: Set the meeting properties

Set the properties in the `cocmdMeetingInfo` class. You just need to provide the
...
```dart
final meetingInfo = cocmdMeetingInfoV2(

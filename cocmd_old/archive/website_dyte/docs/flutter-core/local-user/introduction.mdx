...
By default as soon as you join the meeting, cocmd SDK will produce your video and
audio streams. To change this behaviour, use the `audioEnabled` & `videoEnabled`
parameter

```dart {4,5}
final meetingInfo = cocmdMeetingInfo(
...
If audio and video tracks are disabled during the `cocmdMobileClient`
